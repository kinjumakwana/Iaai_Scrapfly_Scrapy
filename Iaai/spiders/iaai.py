...
class IaaiSpider(ScrapflySpider):
    name = 'iaai'
    base_url = 'https://www.iaai.com/'
    site_url = [ScrapeConfig('https://www.iaai.com/Search?url=QcS%2bxh3YwCeDU1vr3fjt1Dj1kxr2PKw62TMD4KYIpIA%3d')]
    ...
    def start_requests(self):
        ...
        # scrapfly = ScrapflyClient(key='8eff0e8bae724f539949a8001d40015e')
        # result = scrapfly.scrape(ScrapeConfig(
            # url =self.site_url[0].url ),
            # asp=True,
        ...
        yield ScrapflyScrapyRequest(scrape_config=ScrapeConfig(url=self.site_url[0].url), 
                                    callback=self.parse_listing_page,
                             meta={'page_no': page_no, 'payload': payload})
        # wait_until=EC.element_to_be_clickable((By.CLASS_NAME, 'quote'))

    def parse_listing_page(self,  response:ScrapflyScrapyResponse):
        ...
        if detail_urls:
            for url in detail_urls:
                ...
                yield ScrapflyScrapyRequest(
                    scrape_config=ScrapeConfig(url=response.urljoin(self.base_url, url)),
            ...
            yield ScrapflyScrapyRequest(method='POST', scrape_config=ScrapeConfig(url=response.url), callback=self.parse_listing_page,
                                     headers=self.listing_headers, body=json.dumps(payload),
                                     meta={'page_no': page_no, 'payload': payload})

    def parse_detail_page(self, response:ScrapflyScrapyResponse):
        self.log("Wait")
        self.wait_between(MIN_RAND, MAX_RAND)
        # wait = WebDriverWait(self.browser, 10)
